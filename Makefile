...
TOXIPROXY_REDIS_PROXY := "redis_1"
TOXIPROXY_REDIS_TOXIC := redis
...
toxiproxy-inspect:
	$(TOXIPROXY_CLI) inspect $(TOXIPROXY_COOK_PROXY)
	$(TOXIPROXY_CLI) inspect $(TOXIPROXY_POSTGRES_PROXY)
	$(TOXIPROXY_CLI) inspect $(TOXIPROXY_REDIS_PROXY)
...
toxiproxy-add:
	...
	$(TOXIPROXY_CLI) toxic add -n $(TOXIPROXY_REDIS_TOXIC) -t latency -a latency=10 -a jitter=2 $(TOXIPROXY_REDIS_PROXY)
.PHONY: toxiproxy-add
...
toxiproxy-delete:
	...
	$(TOXIPROXY_CLI) toxic delete -n $(TOXIPROXY_REDIS_TOXIC) $(TOXIPROXY_REDIS_PROXY)
...
toxiproxy-fiber:
	...
	$(TOXIPROXY_CLI) toxic update -n $(TOXIPROXY_REDIS_TOXIC) -a latency=10 -a jitter=2 $(TOXIPROXY_REDIS_PROXY)
...
toxiproxy-dsl:
	...
	$(TOXIPROXY_CLI) toxic update -n $(TOXIPROXY_REDIS_TOXIC) -a latency=25 -a jitter=10 $(TOXIPROXY_REDIS_PROXY)
.PHONY: toxiproxy-dsl
